...
def is_duplicated_number(three_digit):
    ...
    result = len(set(str(three_digit))) != 3
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    result = [0, 0]
    for i, d in enumerate(random_number):
        if user_input_number[i] == d:
            result[0] += 1
        elif d in user_input_number:
            result[1] += 1
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            while user_input != '0' and not is_validated_number(user_input):
